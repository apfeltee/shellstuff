...
class RubyUniq
  ...
  def check_opts
    ...
    nocombine.each do |nm|
      if @options.to_h.has_key?(nm) then
  ...
  def seen?(line)
    ...
    if @options.to_h.has_key?(:nocase) then
      cm = cm.downcase
      @options.to_h.delete(:nocase)
    end
    ...
    begin
      ...
      $stderr.printf("error for %p: (%s) %s", cm, e.class.name, e.message)
      return false
    end
  ...
  def do_io(io, filename)
    cache = []
    mustcache = (@options.to_h.key?(:printcount) || @options.to_h.key?(:printfile))
