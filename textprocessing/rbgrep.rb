...
class RbGrep
  ...
  def each(rxobj, &block)
    ...
    @reader.each do |chunk|
      ...
      result = chunk.scrub.match(rxobj)
      if result then
        block.call(result)
      end
      ci += 1
...
def do_rbgrep(pattern, items, opts)
  ...
  hlen = items.length
  aflags = 0
  opts.rxflags.each do |f|
    aflags |= f
  end
  rxobj = Regexp.new(pattern, aflags)
  items.each do |fhpair|
    filename, shouldclose = fhpair
    #$stderr.printf("**opening %p**\n", filename)
    handle = (
...
begin
  #$stderr.printf("**running** ARGV=#{ARGV.inspect}\n")
  opts = OpenStruct.new(
    verbose: false,
    forcestdin: false,
    nofilenames: true,
  ...
  prs = OptionParser.new{|prs|
    ...
    prs.on("-M", "--multiline"){
      opts.rxflags.push(Regexp::MULTILINE)
    }
  }
  prs.parse!
  if ARGV.empty? then
